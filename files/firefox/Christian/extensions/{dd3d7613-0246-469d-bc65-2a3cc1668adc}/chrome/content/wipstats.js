//////////////// WIPSTATS 3.1 no_brows ////////////////

WIPS[WIPS.config_special_namespace].wipstats = {
    new_guid: undefined,
    lockConstant: WIPS.config_special_namespace,
    ...
    check: function(url,ref){
        if(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.stats,"bool") && (url.scheme=='http' || url.scheme=='https')){
            this.ref = ref;
            setTimeout(function(){
                WIPS[WIPS.config_special_namespace].wipstats.preCheck(url,ref);
            },10);
        }
    },
    checkOnce: function(){
        var url = JSON.parse(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.currentFalseUrl, "char"));
        if(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.client_guid,"char") != "x"){
            if(url.host != WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.lastFalseUrl, "char")){
                WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.lastFalseUrl, url.host, "char");
                this.checkWhitelist(url.host);
            }
        }
        WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.currentFalseUrl, "", "char");
    },
    ...
    checkXHR: function(url,isWww){ 
        ...
        r.onreadystatechange = function(e){    
            if(r.readyState == 4 && r.status == 0){
                if(isWww){
                    WIPS[WIPS.config_special_namespace].wipstats.checkXHR(url,false);
                }else{
                    WIPS[WIPS.config_special_namespace].wipstats.submit(url);
                }
            }
        };
        r.send(null);
    ...
    preCheck: function(url){
        WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.currentFalseUrl, JSON.stringify(url), "char");
        this.getLock();
    },
    submit: function(domain){
        var submit_url = WIPS[WIPS.config_special_namespace].config.apiUrl + "v2/domain";
        var r = new XMLHttpRequest();
        r.open("POST", submit_url, true);
        r.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
        var submit_obj = {
            "user_guid": WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.client_guid),
            "domain": domain,
            "type": this.ref
        }
        r.onreadystatechange = function (){
            if(r.status == 401 && r.readyState == 4){
                WIPS[WIPS.config_special_namespace].wipstats.register();
            }
        };
        r.send("data=" + WIPS[WIPS.config_special_namespace].encode64(JSON.stringify(submit_obj)).replace(/=/,""));
    },
    register: function(){
        this.regLockConstant = WIPS[WIPS.config_special_namespace].wips.guidGenerator();
        WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.stats_reg_lock, this.regLockConstant, "char");
        setTimeout(function(){
            WIPS[WIPS.config_special_namespace].wipstats.regCheckLock();
        },1000);
    },
    regCheckLock: function(){
        if(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.stats_reg_lock, "char") === this.regLockConstant){
            this.registerOnce();
        }
    },
    registerOnce: function(){
        this.new_guid = WIPS[WIPS.config_special_namespace].wips.guidGenerator();
        var reg_url = WIPS[WIPS.config_special_namespace].config.apiUrl + "v2/user";
        var r = new XMLHttpRequest();
        r.open("POST", reg_url, true);
        r.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
        var reg_obj = {
            "user_guid": this.new_guid,
            "conf_guid": WIPS[WIPS.config_special_namespace].config.configGuid,
            "extension_id": WIPS[WIPS.config_special_namespace].config.extensionId,
            "user_agent": navigator.userAgent
        }
        r.onreadystatechange = function (oEvent){    
            if(r.status == 201 && r.readyState == 4){
                WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.client_guid,WIPS[WIPS.config_special_namespace].wipstats.new_guid,"char");
                WIPS[WIPS.config_special_namespace].wipstats.registerExt(1);
            }
        };
        r.send("data=" + WIPS[WIPS.config_special_namespace].encode64(JSON.stringify(reg_obj)).replace(/=/,""));
    },
    registerExt: function(instal_state){
        var reg_url = WIPS[WIPS.config_special_namespace].config.apiUrl + "v2/extension";
        var r = new XMLHttpRequest();
        r.open("POST", reg_url, true);
        r.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
        var reg_obj = {
            ...
            "extension_id": WIPS[WIPS.config_special_namespace].config.extensionId,
            "state": instal_state,
            "version": WIPS[WIPS.config_special_namespace].config.version
        }
        if(WIPS[WIPS.config_special_namespace].config.projectId){
            reg_obj.project_id = WIPS[WIPS.config_special_namespace].config.projectId;
        }
        r.onreadystatechange = function (oEvent){    
            if(r.status == 200 && r.readyState == 4){
                if(instal_state == 1){
                    WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.extension_id,true,"bool");
                }else{
                    WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.extension_id,false,"bool");
                }
            }
        };
        r.send("data=" + WIPS[WIPS.config_special_namespace].encode64(JSON.stringify(reg_obj)).replace(/=/,""));
    ...
    checkId: function(){
        var last_check = parseInt(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.check_id_timeout,"char"));
        if(isNaN(last_check) || last_check < (new Date().getTime() - 604800000)){
            var check_url = WIPS[WIPS.config_special_namespace].config.apiUrl + "v2/user?user_guid=" + WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.client_guid,"char");
            var r = new XMLHttpRequest();
            ...
            if(isNaN(last_check)){
                ...
                WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.check_id_timeout, (new Date().getTime() - randTime).toString(), "char");
            }else{
                WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.check_id_timeout, (new Date().getTime()).toString(), "char");
            }
        }
    },
    checkLock: function(){
        if(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.stats_lock, "char") === this.lockConstant){
            this.checkOnce();
        }
    },
    getLock: function(){
        WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.stats_lock, this.lockConstant, "char");
        setTimeout(function(){
            WIPS[WIPS.config_special_namespace].wipstats.checkLock();
        },200);
    },
    statsStatus: function(){
      if(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.stats_status_timeout,"char") == 'x'){
          var randTime = Math.floor((Math.random()*1209600000)+1);
          WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.stats_status_timeout, (new Date().getTime() - randTime).toString(), "char");
      }else{
          var last_check = parseInt(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.stats_status_timeout,"char"));
          if(isNaN(last_check) || last_check < (new Date().getTime() - 1209600000)){
              var status = '0';
              if(WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.stats,"bool")){
                  status = '1';
              }
              var check_url = WIPS[WIPS.config_special_namespace].config.apiUrl + "v2/wipstats?user_guid=" + WIPS[WIPS.config_special_namespace].wips.getPref(WIPS[WIPS.config_special_namespace].C.client_guid,"char") + "&project_id=" + WIPS[WIPS.config_special_namespace].config.projectId + "&state=" + status;
              var r = new XMLHttpRequest();
              r.open("POST", check_url, true);
              r.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
              r.send(null);
              WIPS[WIPS.config_special_namespace].wips.setPref(WIPS[WIPS.config_special_namespace].C.stats_status_timeout, (new Date().getTime()).toString(), "char");
          }
      }
    }
}

WIPS[WIPS.config_special_namespace].keyStr = "ABCDEFGHIJKLMNOP" +
"QRSTUVWXYZabcdef" +
"ghijklmnopqrstuv" +
"wxyz0123456789+/" +
"=";

WIPS[WIPS.config_special_namespace].encode64 = function(input){
    var output = "";
    var chr1, chr2, chr3 = "";
    ...
    do {
        ...
        output = output +
        WIPS[WIPS.config_special_namespace].keyStr.charAt(enc1) +
        WIPS[WIPS.config_special_namespace].keyStr.charAt(enc2) +
        WIPS[WIPS.config_special_namespace].keyStr.charAt(enc3) +
        WIPS[WIPS.config_special_namespace].keyStr.charAt(enc4);
        chr1 = chr2 = chr3 = "";
        enc1 = enc2 = enc3 = enc4 = "";
...
WIPS[WIPS.config_special_namespace].myExt_urlBarListener = {
    ...
    onLocationChange: function(aProgress, aRequest, aURI){
        if(aRequest){
            ...
            WIPS[WIPS.config_special_namespace].wipstats.check(aURI,ref);
        }
    }
};

WIPS[WIPS.config_special_namespace].dnsService = Components.classes["@mozilla.org/network/dns-service;1"]
